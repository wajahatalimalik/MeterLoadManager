...
class MeterRepository(
    ...
    val unreadNotifications = notificationDao.getUnread()
    val unreadCount      = notificationDao.unreadCount()
    ...
    suspend fun changeMeterStatus(meterNo: String, newStatus: MeterStatus,
        ...
        val trigger = when {
            ...
            units != null && units >= BillingCalculator.T_SOFT   -> SwitchTrigger.THRESHOLD_180
    ...
    // ── Notifications ─────────────────────────────────────────────────────────
    suspend fun addNotification(n: AppNotification) = notificationDao.insert(n)
    ...
    suspend fun snoozeNotification(id: Long, hours: Int) =
        notificationDao.snooze(id, System.currentTimeMillis() + hours * 3_600_000L)
    suspend fun deleteNotification(n: AppNotification) = notificationDao.delete(n)
    suspend fun clearDismissed() = notificationDao.clearDismissed()
}
